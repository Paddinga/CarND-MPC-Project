        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    double py = j[1]["y"];
                    double psi = j[1]["psi"];
                    double v_calc = j[1]["speed"];  // speed in mph
                    double v = v_calc * 0.44704;    // speed in m/s
                    double delta = j[1]["steering_angle"];
                    double a = j[1]["throttle"];
                    const double dt = 0.1;          // delay
                    const double Lf = 2.67;
                    
                    // Predicting including delay
                    px = px + v * cos(psi) * dt;
                    py = py + v * sin(psi) * dt;
                    psi = psi + v / Lf * -delta * dt;
                    v = v + a * 10 * dt;
                    
                    // Convert points into car orientation
